...
exports.gerarContratoPDF = (compromisso, contratante, artista, stream) => {
  ...
  // --- CLÁUSULAS DO CONTRATO (Exemplo) ---
  doc.fontSize(14).font('Helvetica-Bold').text('Cláusula 1ª - Objeto do Contrato', { underline: true });
  doc.fontSize(12).font('Helvetica').text(
    `O presente contrato tem como objeto a prestação de serviços artísticos musicais pelo CONTRATADO para o evento "${compromisso.nome_evento}", a ser realizado em ${new Date(compromisso.data).toLocaleDateString('pt-BR')}.`,
    { align: 'justify' }
  );
  doc.moveDown();
  ...
  doc.fontSize(12).font('Helvetica').text(
    `A apresentação ocorrerá em ${compromisso.local}, com início previsto para as ${new Date(compromisso.data).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}h.`,
    { align: 'justify' }
  );
  doc.moveDown();
